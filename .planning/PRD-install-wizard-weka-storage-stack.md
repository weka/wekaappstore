...
**C → No `helm registry login`.**
The quay `dockerconfigjson` secrets placed in the target namespaces are sufficient: the downstream YAML (operator chart + WekaClient) references them via `imagePullSecret`, and chart/image pulls succeed without an explicit `helm registry login`. The operator's existing OCI path (skips `helm repo add` for `oci://` at `:118`) is used as-is — **no operator change required for auth.**
